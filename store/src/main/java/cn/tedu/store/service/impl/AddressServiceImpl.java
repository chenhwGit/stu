...
public class AddressServiceImpl implements AddressService {
	...
	public void addnew(Integer uid, String username, Address address) {
		// 基于参数uid调用countByUid()统计当前用户的收货地址数据的数量
	...
	public List<Address> getByUid(Integer uid) {
		// 执行查询，得到收货地址列表
		List<Address> addresses = findByUid(uid);
		// 遍历查询到的收货地址列表
		for (Address address : addresses) {
			// -- 将不需要响应到客户端的属性设置为null：uid, province_code, city_code, area_code, is_default,
			// 4个日志属性
			address.setUid(null);
	...
	public void setDefault(Integer aid, Integer uid, String username) {
		...
		// 基于参数uid调用updateNonDefaultByUid()将该用户的所有收货地址设为“非默认”
	...
	public Address getByAid(Integer aid) {
		...
		if (address == null) {
			...
			throw new AddressNotFoundException("获取收货地址失败！您尝试访问的数据不存在!");
		}
		// 将不需要对外提供的属性值设置为null
		address.setProvinceCode(null);
		address.setCityCode(null);
